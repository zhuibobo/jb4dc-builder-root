...
public class WorkflowTransformRest {
    ...
    public JBuild4DCResponseVo<PageInfo<ExecutionTaskPO>> getMyProcessTaskListTransform(String modelCategory,int pageSize) throws JBuild4DCGenerallyException {
        ExecutionTaskSearchModel executionTaskSearchModel = buildExecutionTaskSearchModel(modelCategory, pageSize);

        PageInfo<ExecutionTaskPO> executionTaskPOPage=instanceRuntimeRest.getMyProcessTaskList(executionTaskSearchModel).getData();
        return JBuild4DCResponseVo.getDataSuccess(executionTaskPOPage);
    }

    @RequestMapping(value = "/GetMyProcessEndTaskListTransform", method = {RequestMethod.GET, RequestMethod.POST})
    public JBuild4DCResponseVo<PageInfo<ExecutionTaskPO>> getMyProcessEndTaskListTransform(String modelCategory,int pageSize) throws JBuild4DCGenerallyException {
        ExecutionTaskSearchModel executionTaskSearchModel = buildExecutionTaskSearchModel(modelCategory, pageSize);

        PageInfo<ExecutionTaskPO> executionTaskPOPage=instanceRuntimeRest.getMyProcessEndTaskList(executionTaskSearchModel).getData();
        return JBuild4DCResponseVo.getDataSuccess(executionTaskPOPage);
    }

    private ExecutionTaskSearchModel buildExecutionTaskSearchModel(String modelCategory, int pageSize) {
        ...
        return executionTaskSearchModel;
    }
}
